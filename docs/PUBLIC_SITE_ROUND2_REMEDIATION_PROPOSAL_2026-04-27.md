...
- 强化 Relay 详情页的官网地址、联系方式、模型状态说明和模型列表标题。
...
- 问题：详情页顶部公开身份信息应聚焦官网地址与联系方式，Base URL 属于测试和监控字段，不应作为公开详情页身份信息展示；模型健康列表需要清晰标题和口径说明，但不需要额外聚合概览卡。
- 方案：Relay hero meta 只展示官网地址和联系方式；联系信息识别 URL、邮箱、Telegram handle；模型区保留“支持模型”标题、观测口径说明和 7 天状态图例，移除“支持模型健康概览”与健康/异常/最近验证聚合指标。
- 验收：`/relay/aurora-relay` 不显示 `Base URL` 或 `aurora.relaynew.ai/v1`，可见官网地址、联系方式；模型区可见“支持模型”、观测口径说明和逐模型状态，不显示“支持模型健康概览”。
...
- P0：移动端榜单横向溢出。
- P1：Relay 详情公开联系信息与模型状态说明、赞助语言、sitemap、favicon。
