...
test("parseSelfReportedIdentity parses compact JSON responses", () => {
  const result = parseSelfReportedIdentity('{"provider":"Google","model_name":"gemini-2.5-flash","model_version":"gemini-2.5-flash","matches_observed":true}');
...
test("parseSelfReportedIdentity parses fenced JSON responses", () => {
  const result = parseSelfReportedIdentity('```json\n{"provider":"OpenAI","model_name":"gpt-5.4","model_version":null,"matches_observed":false}\n```');
