...
3. Run `./ops/manage-tunnel.sh apply` once after creating the dedicated tunnel so
   Cloudflare knows how to forward origin traffic.
4. Run `./ops/manage.sh deploy` to sync the repo, build the origin image, apply
   migrations, and restart the origin plus `cloudflared`.
5. Run `./ops/manage-edge.sh deploy all` to publish `web`, `api-edge`, and `admin`.
6. Use `./ops/manage.sh status`, `./ops/manage.sh logs`, and
   `./ops/manage.sh health` for ongoing operations.
7. Use `./ops/manage.sh releases` and `./ops/manage.sh rollback` when a release
   needs to be inspected or reverted.

## Notes

- `deploy` is focused on `apps/origin` only. It does not publish `web`, `api-edge`,
...
- The current origin container also joins the shared Docker network from the legacy
  stack so it can reach the existing PostgreSQL container without sharing its tunnel.
